...
public class ContourPipeline extends Pipeline {
  ...
  private Setting<Boolean> drawContours =
      new Setting<>("drawContours", Boolean.class, false, BuiltInWidgets.kToggleSwitch);
  private Setting<Double> normalBoundingRectSolidityMin =
      new Setting<>("normalBoundingRectSolidityMin", Double.class, 0.06, BuiltInWidgets.kTextView);
  private Setting<Double> normalBoundingRectSolidityMax =
      new Setting<>("normalBoundingRectSolidityMax", Double.class, 0.2, BuiltInWidgets.kTextView);
  private Setting<Double> normalBoundingRectAspectRatioMin =
      new Setting<>(
          "normalBoundingRectAspectRatioMin", Double.class, 0.0, BuiltInWidgets.kTextView);
  private Setting<Double> normalBoundingRectAspectRatioMax =
      new Setting<>(
          "normalBoundingRectAspectRatioMax", Double.class, 2.9, BuiltInWidgets.kTextView);
  private Setting<Double> momentYRatioMin =
      new Setting<>("momentYRatioMin", Double.class, 0.45, BuiltInWidgets.kTextView);
  private Setting<Double> momentYRatioMax =
      new Setting<>("momentYRatioMax", Double.class, 999.0, BuiltInWidgets.kTextView);
  ...
  public void process(Mat mat, Context context) {
    ...
    Imgproc.findContours(
        mat, foundContours, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_TC89_L1);
    ...
    for (int i = 0; i < foundContours.size(); i++) {
      ...
      Point normalBoundingRectCenter =
          new Point(normalBoundingRectCenterX, normalBoundingRectCenterY);
      ...
      float normalBoundingRectAspectRatio =
          (float) normalBoundingRect.width / normalBoundingRect.height;
      double normalBoundingRectSolidity =
          Imgproc.contourArea(contour) / (normalBoundingRect.width * normalBoundingRect.height);
      ...
      RectUtil.RectanglePoints minRectPointsSorted =
          RectUtil.findCorners(minRectPoints, normalBoundingRectCenter);
      ...
      if (normalBoundingRectSolidity < normalBoundingRectSolidityMin.get()) {
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F sol min" + normalBoundingRectSolidity,
              normalBoundingRectCenter,
      ...
      if (normalBoundingRectSolidity > normalBoundingRectSolidityMax.get()) {
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F sol max" + normalBoundingRectSolidity,
              normalBoundingRectCenter,
              Core.FONT_HERSHEY_SIMPLEX,
              0.5,
              new Scalar(255, 0, 255),
      ...
      if (normalBoundingRectAspectRatio < normalBoundingRectAspectRatioMin.get()) {
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F a.ra min" + normalBoundingRectAspectRatio,
              normalBoundingRectCenter,
              Core.FONT_HERSHEY_SIMPLEX,
              0.5,
              new Scalar(255, 0, 255),
      ...
      if (normalBoundingRectAspectRatio > normalBoundingRectAspectRatioMax.get()) {
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F a.ra max" + normalBoundingRectSolidity,
              normalBoundingRectCenter,
              Core.FONT_HERSHEY_SIMPLEX,
              0.5,
              new Scalar(255, 0, 255),
      ...
      if (momentYRatio > momentYRatioMax.get()) {
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F mo.y max" + normalBoundingRectSolidity,
              normalBoundingRectCenter,
              Core.FONT_HERSHEY_SIMPLEX,
              0.5,
              new Scalar(255, 0, 255),
      ...
      try {
        ...
        // top left and top right are the poly corners closest to the bounding box (min rect) tl and
        // tr
        Point tl =
            polyList.stream()
                .min(
                    Comparator.comparingDouble(
                        (Point p1) -> distanceBetween(p1, minRectPointsSorted.topLeft)))
                .get();
        Point tr =
            polyList.stream()
                .min(
                    Comparator.comparingDouble(
                        (Point p1) -> distanceBetween(p1, minRectPointsSorted.topRight)))
                .get();
        ...
        if (drawContours) {
          Imgproc.putText(
              mat,
              "F nse" + normalBoundingRectSolidity,
              normalBoundingRectCenter,
              Core.FONT_HERSHEY_SIMPLEX,
              0.5,
              new Scalar(255, 0, 255),
    ...
    Optional<PotentialSmartTarget> potentialTargetOptional =
        potentialTargets.stream().max(Comparator.comparingDouble(o -> o.minRectArea));
  ...
  public static class PotentialSmartTarget {
    ...
    public PotentialSmartTarget(
        Point center,
        RectUtil.RectanglePoints minRect,
        RectUtil.RectanglePoints targetCorners,
        double minRectArea,
        Point matCenter,
        int matWidth,
        int matHeight) {
      ...
      this.degreesHorizontal =
          ((center.x - matCenter.x) / matWidth) * Constants.PSEYE_HORIZONTAL_FOV;
  ...
  public List<Setting<?>> getSettings() {
    return List.of(
        drawContours,
        normalBoundingRectSolidityMin,
        normalBoundingRectSolidityMax,
        normalBoundingRectAspectRatioMin,
        normalBoundingRectAspectRatioMax,
        momentYRatioMin,
        momentYRatioMax);
  }
}
