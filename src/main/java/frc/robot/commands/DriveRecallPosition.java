...
public class DriveRecallPosition extends CommandBase {
  ...
  public void initialize() {
    ...
    // Figure out whether to use forward or backward trajectory, depending on whether they generate
    // properly
    ...
    if (forwardTrajectory != null && backwardTrajectory != null) {
      ...
      // System.out.println("USING DETECTED TRAJECTORY: " + (forwardTrajectory.getTotalTimeSeconds()
      // < backwardTrajectory.getTotalTimeSeconds()));
  ...
  private Trajectory generateTrajectory(Pose2d targetPosition, boolean reversed) {
    try {
      ...
      System.out.println(
          "Failed to generate trajectory to stored position." + " (reversed=" + reversed + ")");
      e.printStackTrace();
    }
    return null;
  }
