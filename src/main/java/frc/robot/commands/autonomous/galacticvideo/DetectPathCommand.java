...
public class DetectPathCommand extends CommandBase {
  ...
  public DetectPathCommand(
      GalacticAutoVideoCommand galacticAutoVideoCommand, VisionSystem visionSystem) {
  ...
  public void execute() {
    ...
    if (blocks.size() >= 2) {
      ...
      galacticAutoVideoCommand.setMessage(
          "[Detect Path] Stable loops at " + stableLoops + " out of " + STABLE_LOOPS_REQUIRED);
    } else {
      stableLoops = 0;
      galacticAutoVideoCommand.setMessage(
          "[Detect Path] Only found " + blocks.size() + " blocks.. Waiting for 2+");
    }
    ...
    if (stableLoops >= STABLE_LOOPS_REQUIRED) {
      ...
      if (largestBlockOptional.isEmpty()) {
        galacticAutoVideoCommand.setMessage(
            "[Detect Path] Largest block optional is empty! Should be impossible here.");
        return;
      }
      int largestArea = visionSystem.calculateArea(largestBlockOptional.get());
      boolean powerCellClose =
          largestArea > GalacticSearchVideo.PIXY_AREA_THRESHOLD_FOR_CLOSE_POWER_CELL;
