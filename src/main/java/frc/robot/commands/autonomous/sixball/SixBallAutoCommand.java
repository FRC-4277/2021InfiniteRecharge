...
public class SixBallAutoCommand extends SequentialCommandGroup {
  ...
  private static final int DESIRED_RPM = 2350;
  ...
  public SixBallAutoCommand(DriveTrain driveTrain, VisionSystem visionSystem, Shooter shooter, VerticalHopper hopper, Intake intake) {
    addCommands(
      // Set start pose
      new ResetOdometryCommand(driveTrain, START_POSE), // will reset NavX too
      // Align & shoot
      new VisionAlignCommand(driveTrain, visionSystem, false, false, false).withTimeout(1.5),
      new ShootAndHopperCommand(shooter, hopper, visionSystem, false, DESIRED_RPM, false).withTimeout(7.0),
      new PrintCommand("Shoot 1 done"),
      // Rotate to 0 & pickup three
      new RotateToCommand(driveTrain, 0).withTimeout(1.5),
      new PickupThreeCommand(driveTrain, visionSystem, intake),
      // Drive back to start
      new LazyRamseteCommand(driveTrain, () -> driveTrain.generateTrajectory(driveTrain.getPose(), START_POSE, MAX_VELOCITY, MAX_ACCEL, false, true, 0, 0)),
      // Align & shoot
