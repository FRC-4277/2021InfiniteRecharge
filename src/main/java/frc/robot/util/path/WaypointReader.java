...
public class WaypointReader {
   ...
   */
  public static TrajectoryGenerator.ControlVectorList getControlVectors(String pathName)
      throws IOException {
    String relativePath = "waypoints/" + pathName + ".path";
    Path path = Filesystem.getDeployDirectory().toPath().resolve(relativePath);

    TrajectoryGenerator.ControlVectorList controlVectors =
        new TrajectoryGenerator.ControlVectorList();
      ...
      while (line != null) {
        ...
        controlVectors.add(
            new Spline.ControlVector(
                ...
                new double[] {
                  Double.parseDouble(split[1]) + PATHWEAVER_Y_OFFSET,
