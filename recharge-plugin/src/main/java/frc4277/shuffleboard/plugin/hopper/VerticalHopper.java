...
public class VerticalHopper extends ComplexData<VerticalHopper> {
     ...
     * @param gateClosed Whether or not the solenoid gate that regulates the 0-th power cell (top most) is closed
     * @param cellsPresent Whether or not individual power cells are present (via sensors)
     * @param speedRunning Whether or not the belt is moving the power cells (positive indicates up towards shooter)
     */
    ...
    public boolean isCellPresent(int index) {
        if (index > MAXIMUM_INDEX) {
            throw new IllegalArgumentException("Maximum index is " + MAXIMUM_INDEX + ", was given " + index);
        }
